最近在做一个合作项目的时候接触到了远程遥控开关这个东西，如下图所示：

![](assets/远程遥控开关是怎样工作的/file-20260211153140593.png)

它不需要编程只需要给好电源，按下遥控器按钮后接收端的对应引脚就会输出高电平，那有了这个电平信号的变化，我们就可以用一个单片机来检测这个信号，做任何想做的事情了，比如控制灯的亮灭，控制舵机的转动等简单或需要一定操作逻辑的事情。

想要知道这是如何实现的，我们要先搞懂他们是如何发送信号的。这就引出了一个概念叫调制技术。



# 调制技术

调制技术可分为四类：**模拟调制、数字调制、脉冲调制、扩频调制**。

**模拟调制**：常用于AM、FM广播中。

**数字调制**：二进制信号（0 和 1）的传输。

**脉冲调制**：用脉冲参数变化来传输信息

**扩频调制**：用扩展频带的方法提高抗干扰能力

这次所用到的遥控开关就是数字调制。

## ASK：Amplitude Shift Keying 幅度键控

通过改变载波振幅来传输数字信号的调制方式。
...
以载波频率为基准，通过改变频率来表示不同的数字信号。

例如在最简单的二进制FSK中：

当发送数字 **1** 时，载波频率向上偏移（较高频率）

当发送数字 **0** 时，载波频率向下偏移（较低频率）

也就是说：

数字信息不是通过改变振幅，而是通过改变“频率”来传输。

![](assets/远程遥控开关是怎样工作的/file-20260211145923441.png)

## PSK： Phase Shift Keying 相位偏移

通过改变载波**相位**来表示数字信息的调制方式。

在保持载波振幅和频率不变的情况下，通过改变信号的相位来传输数据

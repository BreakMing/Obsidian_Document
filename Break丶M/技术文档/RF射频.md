...
其中433指代的是通信频率   14152657是接收到的数据，这个数据是十进制的，转换成二进制为1101 0111 1111 0011 1101 0001

转换后的二进制是24bit的   前20个bit代表的是发送端的ID，后四个bit代表的是发送的数据，通过读取到的数据和数据手册中键位组合表就可以知道这个哪个按键
